...
class SemanticScholarService
  ...
  def self.get_paper_details(paper_id)
    response =
      BaseService.semantic_scholar_connection.get(
  ...
  def self.get_paper_references(paper_id)
    ...
    required_call_count =
      citation_calls_required(
        SemanticScholarFacade.get_paper_details(paper_id).citation_count
      )
    ...
    required_call_count.times do |index|
      response =
        BaseService.semantic_scholar_connection.get(
          ...
          default_params.merge({ limit: 1000 }, { offset: index * 1000 })
        )
      ...
      json_response[:data].each { |paper_data| response_arr << paper_data }
